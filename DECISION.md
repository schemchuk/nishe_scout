...
**Exact customer query / intent:** all 6 keyword-based intents researched across Phase C–G are closed (Sections 0.8, 0.10). Phase H's 3 operational-pain problem classes have now been through Pain/Switching/Wedge testing in Phase I (Section 0.12): **Confluence stale/outdated content detection & review cadence SURVIVES**; **orphaned digital artifacts after user/project lifecycle events (Jira) is NEEDS EVIDENCE** (real buyer/pain/native-gap but zero proven paid transactions); **issue-level data quality/hygiene drift after creation (Jira) is KILLED** (diffuse buyer, indirect pain, zero transactions across 3 independent attempts).

**Candidate product:** none chosen — 2 concrete wedges carried forward (see Section 0.12), neither designed into an MVP or architecture yet, per instruction not to pick a winner by score

**One-sentence wedge:** the strongest candidate is a **Confluence per-owner review-cadence digest** (Owner + Review Date properties Confluence lacks natively, one consolidated notification per owner per cycle instead of one per page, persistent until confirmed, available on Free/Standard tier) — backed by a self-identified buyer (Knowledge Manager), an explicit admin-set review cadence, and a directly adjacent paid category (Confluence approval/governance apps) with 983-1,700 installs proving real budget exists; a secondary, not-yet-proven candidate is a **Jira orphaned-ownership cleanup sweep** for departed users' filters/dashboards/issues/project-leads
...
| **Clear wedge** | **PASS (1 of 3 Phase H classes) — Confluence Review Cadence Digest** | Original 2 hypotheses killed in Phase E; Phase F's 3 replacements killed in Phase G. Phase H found 3 operational-pain problem classes; Phase I (Section 0.12) ran Pain/Switching/Wedge testing on all 3 — 1 SURVIVES (Confluence stale-content/review-cadence), 1 NEEDS EVIDENCE (Jira orphaned-ownership cleanup, real pain but zero proven paid transactions), 1 KILLED (issue-level data quality drift). RESEARCH.md → Phase E, Phase F, Phase G, Phase H, Phase I |
| Solo feasibility | LOW-MEDIUM risk for the surviving wedge | Confluence Review Cadence Digest: LOW support burden (notification/reporting only, no destructive bulk action), LOW API/policy risk (internal Confluence content API), HIGH Architecture Fit (Forge-native, precedented by 4 existing small apps). Jira Departure Sweep (secondary, NEEDS EVIDENCE): MEDIUM support burden (bulk reassignment carries mis-assignment risk). RESEARCH.md → Phase I |
| Support burden | LOW (primary wedge) / MEDIUM (secondary wedge) | RESEARCH.md → Phase I, Support Gate classification per class |
...
**Reason:** Phase A/B0 selected Atlassian Marketplace (Section 0). Phase B1→C→D→E killed both surviving keyword-based wedges (Section 0.8); Phase F's 3 replacements were killed in Phase G (Section 0.10). Phase H (Section 0.11) abandoned keyword-sweeping and discovered 3 operational-pain problem classes by working backward from real 2026 Marketplace apps, narrowing 10 candidate classes down to 3 via native-capability and saturation screening. Phase I (Section 0.12) then ran full Pain/Switching/Wedge testing on those 3 — collecting buyer roles, trigger/frequency, current workarounds, economic consequences, real pricing, and 30 total dated customer quotes across the 3 classes, all sourced from Atlassian Community threads and Marketplace listings, none fabricated. **Result: 1 of 3 SURVIVES.** Confluence stale/outdated content detection & review cadence has every required link in the chain confirmed simultaneously: a self-identified buyer ("i am Knowledge Manager here"), an explicit admin-set review cadence, real adjacent paid behavior (983-1,700 installs in the directly-adjacent Confluence approval/governance category), a credible reason to pay (native Automation's one-email-per-page design causes review reminders to silently fail at scale — "50 expired pages... 50 separate emails" — plus an explicit case of a user comparison-shopping toward a paid plugin), a native gap confirmed on 3 independent points, LOW support burden, and a still-open wedge angle (a per-owner digest with Free/Standard-tier availability, not explicitly claimed by any of the 4 tiny existing competitors). Orphaned digital artifacts after user/project lifecycle events (Jira) is `NEEDS EVIDENCE` — real buyer, 7-year-confirmed native gap, and a direct willingness-to-pay quote, but zero installs on either direct competitor and its cleanest wedge angle already claimed by a same-week rival. Issue-level data quality/hygiene drift after creation (Jira) is `KILLED` — not by native capability but by a diffuse buyer, indirect pain evidence overlapping already-rejected themes, and zero transactions across 3 independent 2026 attempts. The stop condition (`ATLASSIAN WEDGE SEARCH FAILED`) was explicitly checked and does **not** apply, since Class 3 clears every required bar on its own. This stays `NEEDS EVIDENCE` rather than `BUILD` because, per instruction, no winner was chosen by score and neither of the 2 carried-forward wedges (Confluence Review Cadence Digest; Jira Departure Sweep) has been designed into an MVP, scoped for economics, or architected — that is the next required step, not yet started.
