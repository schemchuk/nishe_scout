...
**Exact customer query / intent:** the 3 intents researched in Phase C/D/E (`gdpr compliance confluence`, `hubspot integration jira`, `confluence page views analytics`) are closed — wedge-tested and killed in Phase E, or moved to HOLD (see Section 0.8). Phase F (Section 0.9) replaced them with 3 new candidate intents pending Pain/Switching research: `enforce definition of done jira`, `prevent issue reopening jira`, `compare confluence page versions`.

**Candidate product:** none — the 3 Phase F intents are pre-Pain/Switching-research candidates, not products; no MVP or architecture has been designed for any of them

**One-sentence wedge:** none confirmed yet — Phase F identified 3 candidate intents with real but incomplete demand signals (repeated weak-execution attempts, tracking-vs-gating ambiguity, and native-feature-overlap risk respectively); the next phase must establish pain + switching evidence before any wedge claim
...
| **Clear wedge** | **FAIL (prior 3 intents); NEEDS EVIDENCE (new 3 intents)** | Both original candidate hypotheses killed in Phase E. Phase F (Section 0.9) surfaced 3 new candidate intents with partial, unresolved demand signals — none confirmed as a wedge yet; Pain/Switching research is the next required step. RESEARCH.md → Phase E, Phase F |
| Solo feasibility | Moot — no surviving wedge to assess | HubSpot↔Jira hypothesis carried a HIGH support-trap rating before being killed on its own terms (job already served); `prevent issue reopening jira` (Phase F) is flagged HIGH support-burden risk pending further evidence. RESEARCH.md → Phase D, Phase E, Phase F |
| Support burden | Moot — no surviving wedge to assess | RESEARCH.md → Phase D, Phase E, Phase F |
...
**Reason:** Phase A (marketplace screening) and Phase B0 (cold-start validation) are complete — Atlassian Marketplace is selected (see Section 0). Phase B1 (search landscape) is complete — 5 candidate search intents selected (see Section 0.5). Phase C (demand validation) is complete — 3 of those 5 intents carried forward (see Section 0.6). Phase D (pain + switching) identified 2 pain/wedge hypotheses (see Section 0.7). Phase E (wedge kill test) is complete — **both hypotheses were killed**: in each case, the exact job is already delivered by 3–7 active, funded competitors, so no defensible wedge exists for either (see Section 0.8). Per AGENTS.md §5, a missing wedge is a veto that is not compensated by the real demand evidence found in Phase C. `confluence page views analytics` is on `HOLD` pending a concrete unmet job. Phase F (fresh keyword expansion, see Section 0.9) is now also complete — 35 new customer-intent queries were checked, 15 rejected outright as saturated by established incumbents, and 3 new candidate intents (`enforce definition of done jira`, `prevent issue reopening jira`, `compare confluence page versions`) were narrowed down and carried forward, each with an explicit open question still to be resolved (execution-quality gap vs. real willingness to pay; hard-gate vs. visibility-only demand; and native-platform-feature overlap, respectively). No product has been chosen and none is being pursued on Atlassian Marketplace right now. This is `NEEDS EVIDENCE` rather than `REJECT` because the marketplace itself (Section 0) remains validated and Phase F surfaced 3 non-saturated candidate intents with partial demand signals — what is still missing is pain + switching evidence for these 3, which is the next required step, not abandonment of the marketplace.
